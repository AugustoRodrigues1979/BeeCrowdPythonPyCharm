from cedulas_1021.cedulas import run_challenge
...
def show_progress(actual_value, target_value, first_trace):
    ...
    progress = progress + f'Progress:{0.00:.02f}% ' # Set progress in porcentage
    progress = progress + f'[Processed Numbers:{round(actual_value, 0):.0f}/{target_value:.00f}] ' # Set Amount Numbers processed
...
class TestCedulas:
    ...
    def test_amount_between_0_00_to_1_000_000_00(self):
        ...
        last_porcentage = 0.00 #Initialize last porcentage
        while actual_value <= target_value: #Run until the all amounts is processed
            ...
            if actual_value == 0: #Check if its is first iteration
                print() #Skip one line
                show_progress(actual_value, target_value, True) #Show line progress
            else: #Run for other iteractions
                progress = round((actual_value / target_value) * 100, 2) #Set actual progress in porcentage
                if round(progress % 0.01) == 0 and last_porcentage != progress: #Check if occured rise percentage from last percentage
                    show_progress(actual_value, target_value, False) #Show line progress
                    last_porcentage = progress #Store last percentage showing for user
